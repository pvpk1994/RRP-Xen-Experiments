...
path_30<-paste(wd_dir,"/credit_3_7_raw_result/172.23.56.212_0.99_30.log",sep="")
print(path_30)
...
max_result <- max(ddl_subtract_res)

# gather everything into one vector
total_res <- data.frame("Scheduler_name"="credit", "mean" = mean_result,
                        "median" = median_result, "Max" = max_result)

print(total_res)

# writing to the file
write.table(total_res, file="latency_result.csv", row.names = FALSE, 
